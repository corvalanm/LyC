%{
    ...
    int determinar_tipoDeDato(int tipo1, int tipo2);
    int compatibilidad_datos(int tipo1, int tipo2);
...
iteracion: WHILE PAR_ABR condicion PAR_CIE DO programa ENDWHILE {
    ...
    | WHILE ID {strcpy(_id_while_especial, $2);} IN PAR_ABR lista_exp PAR_CIE DO programa ENDWHILE {  
        ...
        int k = 0;
        agregar_simbolo("_0", INT_CONST, (void*)&k);
        k = 1;
        agregar_simbolo("_1", INT_CONST, (void*)&k);
        iteracion_ptr = crearNodo("WHILE_ESP", crearNodo("COND_W_ESP", lista_exp_ptr, crearNodo("==", crearHoja("1"), crearHoja("_ejec_while_esp"))), pila_programa[--pila_programa_tope] );
    }
...
lista_exp: lista_exp COMA expresion {
        ...
        // Si la expresion es igual al ID del while le asigna 1 a _ejec_while_esp.
        temp_lista_expr = crearNodo("IF", crearNodo("==", crearHoja(_id_while_especial), expresion_ptr), crearNodo(":=", crearHoja("_ejec_while_esp"), crearHoja("1")));
        lista_exp_ptr = crearNodo("LISTA_EXP", lista_exp_ptr, temp_lista_expr);
    }
    | expresion {
        //printf("\nRegla 9.1: lista_exp -> expresion");
        // Inicializa la variable _ejec_while_esp = 0 y si la expresion es igual al ID del while le asigna 1 a _ejec_while_esp
        temp_lista_expr = crearNodo("IF", crearNodo("==", crearHoja(_id_while_especial), expresion_ptr), crearNodo(":=", crearHoja("_ejec_while_esp"), crearHoja("1")));
        temp_lista_expr = crearNodo("BLOQUE", crearNodo(":=", crearHoja("_ejec_while_esp"), crearHoja("0")), temp_lista_expr);
...
asignacion: ID ASIG expresion { 
        ...
        if(compatibilidad_datos(buscar_tipo($1), expresion_ptr->tipo) == -1){yyerror("No se puede realizar la asignacion porque son de diferentes tipos de datos");YYABORT;};
        
        asignacion_ptr = crearNodo(":=", crearHoja($1), expresion_ptr);
        asignacion_ptr->tipo = expresion_ptr->tipo;
    }
    | ID ASIG longitud {
        printf("\nRegla 11: Asignacion");
        if(buscar_simbolo($1, VARIABLE_NUM) == -1){yyerror("Variable desconocida");YYABORT;};
        if(compatibilidad_datos(buscar_tipo($1), TIPO_INTEGER) == -1){yyerror("No se puede realizar la asignacion porque son de diferentes tipos de datos");YYABORT;};
        
        asignacion_ptr = crearNodo(":=", crearHoja($1), longitud_ptr);
        asignacion_ptr->tipo = TIPO_INTEGER;
    }
...
cond_s : factor {aux_factor_ptr = factor_ptr;} MAYOR factor {
        if(compatibilidad_datos(aux_factor_ptr->tipo, factor_ptr->tipo) == -1){yyerror("Tipo de datos incompatibles");YYABORT;};
...
expresion: expresion MAS termino {
        printf("\nRegla 16.1: SUMA ");
        if((tipo_aux = determinar_tipoDeDato(expresion_ptr->tipo, termino_ptr->tipo)) == -1){yyerror("Tipo de datos incompatibles");YYABORT;};

        expresion_ptr = crearNodo("+", expresion_ptr, termino_ptr);
        expresion_ptr->tipo = tipo_aux; 
    }
    ...
    | termino {
        printf("\nRegla 16.3: expresion -> termino ");
        expresion_ptr = termino_ptr;
        expresion_ptr->tipo = termino_ptr->tipo;
    }

/*Regla n17*/
termino: termino MULT factor {
        printf("\nRegla 17.1: MULTIPLICACION ");
        if((tipo_aux = determinar_tipoDeDato(termino_ptr->tipo, factor_ptr->tipo)) == -1){yyerror("Tipo de datos incompatibles");YYABORT;};

        termino_ptr = crearNodo("*", termino_ptr, factor_ptr);
        termino_ptr->tipo = tipo_aux;
    }
    | termino DIV factor {
        printf("\nRegla 17.2: DIVISION ");
        if((tipo_aux = determinar_tipoDeDato(termino_ptr->tipo, factor_ptr->tipo)) == -1){yyerror("Tipo de datos incompatibles");YYABORT;};
    ...
    | factor {
        printf("\nRegla 17.3: termino -> factor ");
        termino_ptr = factor_ptr;
        termino_ptr->tipo = factor_ptr->tipo;
    }
...
factor: PAR_ABR expresion PAR_CIE 
    {//printf("\nRegla 18.1: factor -> PAR_ABR expresion PAR_CIE ");
        ...
        factor_ptr->tipo = expresion_ptr->tipo;
    }
    | ID 
...
longitud: LONG PAR_ABR contar PAR_CIE 
    {
        ...
        sprintf (_aux_str, "%d",_longitud_total);
        longitud_ptr = crearHoja(_aux_str);
        longitud_ptr->tipo = TIPO_INTEGER;

        sprintf (_aux_str, "_%d",_longitud_total);
        agregar_simbolo(_aux_str, INT_CONST, (void*)&_longitud_total);
    }
...
int determinar_tipoDeDato(int tipo1, int tipo2) {
    if(tipo1 == TIPO_STRING && tipo2 != TIPO_STRING || tipo1 != TIPO_STRING && tipo2 == TIPO_STRING)
        return -1;

    if(tipo1 == TIPO_STRING && tipo2 == TIPO_STRING) 
        return TIPO_STRING;

    if(tipo1 == TIPO_INTEGER && tipo2 == TIPO_INTEGER) 
        return TIPO_INTEGER;
...
int compatibilidad_datos(int tipo1, int tipo2) {
    if(tipo1 == TIPO_STRING && tipo2 == TIPO_STRING || tipo1 == TIPO_INTEGER && tipo2 == TIPO_INTEGER || tipo1 == TIPO_REAL && tipo2 == TIPO_REAL)
        return 1;
